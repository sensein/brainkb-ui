...
export default function ORCID<P extends ORCIDProfile>(
  ...
  return {
    ...
    authorization: {
      params: {
        scope: "openid email profile", // Request OpenID and limited read access
      },
    },
    idToken: true,
    checks: ["pkce", "state"],
    userinfo: {
      ...
      async request({ client, tokens }) {
        ...
        profile.name =
          profile.name ||
          `${profile.given_name || "Unknown"} ${profile.family_name || ""}`.trim();
    ...
    profile(profile) {
      return {
        ...
        name: profile.name || "Unknown User", // Full name or fallback
        email: profile.email || "no-email-provided@orcid.org", // Email or fallback
